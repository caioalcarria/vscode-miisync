...
import { PathMappingManager } from "./pathmapping.js";
 ...
 * First tries to use path mapping if available, then falls back to config-based calculation
 */
export function GetRemotePath(
  filePath: string,
  { remotePath, removeFromLocalPath }: UserConfig,
  addWeb = true
) {
...
/**
 * Versão assíncrona que tenta usar o mapeamento de caminhos primeiro
 * Se encontrar mapeamento, usa exatamente o caminho salvo no arquivo, ignorando configurações
 */
export async function GetRemotePathWithMapping(
  ...
  return GetRemotePath(filePath, userConfig, addWeb);
}
...
export async function ValidatePath(
  ...
  if (config.include) {
    ...
    if (include.ignores(relativePath)) return true;
  }
...
export async function GetAllFilesInDirTree(
  startPath: string
): Promise<SimpleFolder> {
  if (!(await exists(startPath))) {
    return null;
  }
  ...
  for (const file of directory) {
    ...
    if (stat.isDirectory()) {
      promises.push(
        GetAllFilesInDirTree(name).then((cFolder) => {
...
export async function PrepareUrisForService(
  uris: Uri[]
): Promise<SimpleFolder> {
  /**
  ...
  let rootFolderPath: string = (await lstat(uris[0].fsPath)).isDirectory()
    ? uris[0].fsPath
    : path.dirname(uris[0].fsPath);
    ...
    if (!hasParent) {
      ...
      if (directoryPath != rootFolderPath) {
        if (IsSubDirectory(directoryPath, rootFolderPath)) {
          ...
          const relation =
            path
              .relative(rootFolderPath, directoryPath)
              .match(/\.\./g)
              ?.join("/") || "";
      ...
      if (folder) folder.uris.push(uriX);
      else {
   ...
   */
  const rootFolder: SimplePreFolder = {
    folders: [],
    files: [],
    uris: [],
    path: rootFolderPath,
  ...
  for (let index = 0; index < uriFolders.length; index++) {
    ...
    const folders = path
      .relative(rootFolder.path, uriFolder.path)
      .split(path.sep);
    ...
    if (folders.length != 1 || folders[0] != "") {
      for (let depth = 0; depth < folders.length; depth++) {
        ...
        const index = depthFolder.folders.findIndex(
          (fFolder) => fFolder.path == folderPath
        );
        if (index != -1) {
