...
async function updatePathMappingForNewFile(
  ...
  try {
    ...
    if (mappingInfo) {
      ...
      const existingMapping = config.mappings.find(
      ...
      if (!existingMapping) {
        ...
        await PathMappingManager.addMapping(
          rootPath,
          relativePath,
          remotePath,
          content
...
async function verifyUploadIntegrity(
  ...
  try {
    ...
    if (!serverResponse || IsFatalResponse(serverResponse)) {
      console.error(
        "Erro ao baixar arquivo do servidor para verificação:",
        IsFatalResponse(serverResponse)
          ? serverResponse.Rowsets.FatalError
          : "Resposta inválida"
      );
      return false;
    }
    ...
    const payload = serverResponse?.Rowsets?.Rowset?.Row?.find(
    ...
    if (contentsMatch) {
      window.showInformationMessage(`✅ Upload verificado: "${fileName}"`, {
        detail: `Arquivo confirmado no servidor: ${remotePath}`,
      });
    } else {
      window.showWarningMessage(`⚠️ Verificação falhou: "${fileName}"`, {
      ...
      console.log(
        "Primeiros 100 chars servidor:",
        serverContent.substring(0, 100)
      );
    }
    ...
    const fileName = path.basename(localFilePath);
    window.showErrorMessage(`❌ Erro na verificação: "${fileName}"`, {
...
export async function UploadFile(
  ...
  const upload = async () => {
    ...
    try {
      ...
      window.showInformationMessage(
        `🔍 Verificando integridade: "${fileName}"...`
      );

      const isVerified = await verifyUploadIntegrity(
        uri.fsPath,
        sourcePath,
        content,
        system
      ...
      if (!isVerified) {
        window.showErrorMessage(`❌ Verificação falhou: "${fileName}"`, {
          detail: `Upload concluído em ${duration}ms, mas o arquivo no servidor não confere com o local`,
        });
        return {
          aborted: false,
          error: true,
          message: "Upload concluído mas verificação falhou",
        };
      } else {
        console.log(
          `✅ Upload verificado com sucesso em ${duration}ms: ${fileName}`
        );

        // Atualiza o mapeamento se estiver em um diretório mapeado
        try {
          await updatePathMappingForNewFile(uri.fsPath, sourcePath, content);
          // Reset estado de modificação no local-files-mapping.json (se existir entrada)
          try {
            const mapped = localFilesMappingManager.getFile(uri.fsPath);
            if (mapped) {
              // Recalcula hash atual para virar baseline
              ...
              mapped.serverModified = new Date(); // sem metadata remota específica disponível
              // Persistir salvando via addOrUpdateFile para manter consistência
            ...
            console.warn(
              "⚠️ Falha ao resetar estado no mapping pós-upload:",
              e
            );
          }
